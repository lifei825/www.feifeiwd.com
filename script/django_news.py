...
class djnew(object):
    ...
    def getnew(self):
        ...
        newdate=re.findall('''发布于 (\d{4}-\d{2}-\d{2})''',webdata)
        getlist=re.findall(r'''(http://.*?.oschina.net/.*?)' target='_blank'>(.*?)</a></h3>''',webdata)
        weblist=[];n=0
        for i in getlist:
            url,title=i[0],i[1]
        ...
        return weblist
    def toredis(self):
        r9.delete(self.keyword)
        for url,tit,nd in self.getnew():
            r9.rpush(self.keyword,(tit,url,nd))
